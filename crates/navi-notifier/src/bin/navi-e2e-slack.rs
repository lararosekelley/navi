...
async fn run() -> Result<(), String> {
    ...
    // Bound the read-back to messages from this run: PIDs are recycled u32s, so a
    // prior run's identical marker could still sit in recent history and pass the
    // test without this run delivering anything.
...
/// Whether the channel's history since `oldest` (a Unix timestamp) contains a message
/// mentioning `marker`.
