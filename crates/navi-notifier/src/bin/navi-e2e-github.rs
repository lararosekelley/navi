...
async fn seed(
    ...
    .await?;
    // PID-scoped filename so it can't collide with a file already on the default
    // branch (which would make the Contents API demand a `sha` and 422 the seed).
    put(
        http,
        &format!("{repo}/contents/navi-e2e-{}.txt", std::process::id()),
        actor_token,
        &json!({
...
async fn verify(
    ...
    let source = GitHubSource::new(GitHubSourceConfig {
        ...
        // Honor E2E_GITHUB_API (GHE): only the public API maps to the source's default.
        api_base: (api != "https://api.github.com").then(|| api.to_string()),
        track_prs: false,
        mark_read: false,
