...
async fn cmd_run(config_path: &Path) -> Result<()> {
    ...
    // Housekeeping, not polling: daily is ample for a horizon measured in months,
    // and the sweep VACUUMs when it removes anything. `None` runs it once at
    // startup, so a daemon restarted more often than daily still prunes.
    ...
    loop {
        ...
        if last_prune.is_none_or(|t| t.elapsed() >= PRUNE_INTERVAL) {
            match store.prune(config.general.state_retention_days).await {
                Ok(pruned) if pruned.cursors > 0 => info!(
                    cursors = pruned.cursors,
                    retention_days = config.general.state_retention_days,
                    "pruned poll bookkeeping for long-quiet PRs"
                ),
                Ok(_) => {}
