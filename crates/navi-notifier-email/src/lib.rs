...
/// A plain-text, one-line headline (no markup; the HTML body escapes it as a whole).
fn headline(event: &Event) -> String {
    event.headline(event.actor_label(), |s| s.to_string())
}
...
fn body_html(event: &Event, headline: &str, repo_ref: &str, link: &str) -> String {
    let title = html_escape(&event.pull_request.title);
    let mut out = format!(
        "<p><strong>{}</strong></p>\n<p><a href=\"{}\">{} {}</a></p>\n",
...
mod tests {
    use super::*;
    use navi_notifier_core::model::{Actor, EventKind, PullRequest, Repo, ViewerRelationship};
    ...
    fn threads_events_for_the_same_pr() {
        ...
        // Each message's own Message-ID is distinct from the shared thread id.
