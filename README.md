...
cursors for PRs navi hasn't looked at in `general.state_retention_days` are swept once a day by `navi run`, so they
don't accumulate a row per PR it has ever seen. Rows for settled PRs stay gone; a still-open but quiet PR is
re-fetched once per retention period, which costs a few API calls and no alerts.
